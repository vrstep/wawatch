...
import (
	...
	"github.com/vrstep/wawatch-backend/client"
)

var animeClient *client.AnimeClient

func init() {
	// Configure with the anime service URL
	animeClient = client.NewAnimeClient("http://localhost:8081")
}

// SearchAnime handles anime search requests by forwarding to the anime service
func SearchAnime(c *gin.Context) {
	...
	// Get request ID to forward
	requestID, exists := c.Get("RequestID")
	if exists {
		animeClient.ForwardRequestID(requestID.(string))
	}

	// Use client to call anime service
	results, total, err := animeClient.SearchAnime(query, page, perPage)
	if err != nil {
...
func GetAnimeDetails(c *gin.Context) {
	...
	// Use client to call anime service
	anime, err := animeClient.GetAnimeDetails(id)
	if err != nil {
...
// GetPopularAnime fetches popular anime from the anime service
func GetPopularAnime(c *gin.Context) {
	...
	// Use client to call anime service
	results, total, err := animeClient.GetPopularAnime(page, perPage)
	if err != nil {
...
// GetTrendingAnime fetches trending anime from the anime service
func GetTrendingAnime(c *gin.Context) {
	...
	// Use client to call anime service
	results, total, err := animeClient.GetTrendingAnime(page, perPage)
	if err != nil {
...
// GetAnimeBySeason fetches anime for a specific year and season from the anime service
func GetAnimeBySeason(c *gin.Context) {
	yearParam := c.Param("year")
	seasonParam := c.Param("season") // WINTER, SPRING, SUMMER, FALL
	...
	// Use client to call anime service
	results, total, err := animeClient.GetAnimeBySeason(year, seasonParam, page, perPage)
	if err != nil {
...
func GetAnimeRecommendations(c *gin.Context) {
	...
	// Use client to call anime service
	results, total, err := animeClient.GetPopularAnime(page, perPage) // Placeholder
	if err != nil {
